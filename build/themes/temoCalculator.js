export class TempoCalculator {
    ...
    calculateDevelopmentTempo() {
        ...
        const oppDeveloped = this.countDevelopedPieces(this.side === "w" ? "b" : "w");
    ...
    calculateInitiativeTempo() {
        ...
        const spaceAdvantage = this.currentSideState.spaceScore.spaceadvantage / 50;
        return tacticalDiff + spaceAdvantage;
    }
    calculateAttackTempo() {
        ...
        const kingSafetyDiff = oppKingSafety.kingsafetysadvantage - ourKingSafety.kingsafetysadvantage;
        // Attacking opponent's king with more pieces = tempo advantage
        const attackerDiff = oppKingSafety.attackerscount - ourKingSafety.attackerscount;
        // Normalize
        return kingSafetyDiff / 50 + attackerDiff * 0.3;
    }
    calculateMobilityTempo() {
        const mobilityAdvantage = this.currentSideState.pieceMobilityScore.mobilityadvantage;
    ...
    countDevelopedPieces(color) {
        ...
        developed += placement.knightplacement.filter((sq) => !sq.endsWith(startingRank)).length;
        // Count bishops developed
        developed += placement.bishopplacement.filter((sq) => !sq.endsWith(startingRank)).length;
        // Count rooks developed (off starting rank, but heavy penalty for early development)
        const developedRooks = placement.rookplacement.filter((sq) => !sq.endsWith(startingRank)).length;
        developed += developedRooks * 0.5; // Rooks count less until later
        // Queen developed too early can be negative in opening
        const queenDeveloped = placement.queenplacement.some((sq) => !sq.endsWith(startingRank));
        ...
        const advancedPawns = placement.pawnplacement.filter((sq) => !sq.endsWith(pawnRank)).length;
