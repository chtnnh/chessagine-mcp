...
import { getLichessMasterOpeningStats, getLichessOpeningStats, getLLMTranslator, } from "../tools/opening.js";
...
export class LichessService {
    ...
    async getLichessMasterGamesOpeningBook(fen) {
    ...
    async getLichessPublicGamesOpeningBook(fen) {
    ...
    async getLichessUserRecentGames(username) {
    ...
    lichessGameIdValidator(gameUrlOrId) {
    ...
    async getLichessGamePgnById(gameUrlOrId) {
    ...
    async getLichessPuzzleSession(themes, ratingFrom, ratingTo) {
    ...
    getLichessUsername() {
        return {
            username: this.LICHESS_USERNAME.length === 0
                ? "No Username Found"
                : this.LICHESS_USERNAME,
        };
    }
    async getLichessUserStudies(username) {
...
- Updated: ${updated}`;
    ...
    async getLichessStudyPgn(studyId) {
        const authToken = this.LICHESS_STUDY_TOKEN;
        if (!authToken || authToken.length === 0) {
            return {
